...
static void Cal_Inf_2D(TEMP& Temp);
...
void Output_LamEff1D_To_File(double lamEff_1D);
void Output_LamEff2D_To_File(void);
/********************************************************************************************************************************
** Function name:       Cal_LamEff1D_LamEff2D_Parm
** Descriptions:        这个函数是总的，计算lamEff1D（输出到文件中）
						以及lamEff2D,parm of lamEff2D的值（存入g_Spd.lamEff_Parm数组中，[0]对应的为lamEff2D,[1]对应为parm）
...
void Cal_LamEff1D_LamEff2D_Parm(void)
{
	...
	Init_Temp(Temp);
	lamEff_1D = Cal_LamEff_1D(Temp);
	Output_LamEff1D_To_File(lamEff_1D);

	Cal_LamEff_And_Parm(Temp);
	Output_LamEff2D_To_File();
}
/********************************************************************************************************************************
...
void Cal_Effective_Rate_And_Output_Into_File(void)
{
	...
	double x2;
	double aveRec_Rate = 1.0 / (g_Spd.BETA_REC * Gamma_Fun1(x1));		//set_parms.cpp中已经固定了平均恢复速率的值
	double temp;
	...
	for (double effDif_Rate = (LAMEFF_MIN); effDif_Rate < ((LAMEFF_MAX)+((0.001)*(LAMEFF_DELTA))); effDif_Rate += (LAMEFF_DELTA))
	{
		...
		g_Spd.lamEff_Parm[1].push_back(temp);		//beta_inf
	}
	...
	double& parmSol, double& lamEff_Sol)
{
	...
	solL = Cal_LamEff_2D(Temp) - lamEff;
	Update_Var_Of_Temp(Temp, right);
	solR = Cal_LamEff_2D(Temp) - lamEff;
	...
	else									//否则，一直二分法直到得出最后的结果
	{
		while (1)
		{
			...
			solM = Cal_LamEff_2D(Temp) - lamEff;
...
** Descriptions:        由于这里我们只改变ALPHA_INF_2D或者BETA_INF_2D（2维的），所以更新的时候只要改变inf_2D和ita_2D的值
...
void Update_Var_Of_Temp(TEMP& Temp, double parm)
{
	...
	g_Spd.ALPHA_INF_2D = parm;
#elif defined PARM_BETA_INF_2D
	g_Spd.BETA_INF_2D = parm;
#endif

	vector<double>().swap(Temp.inf_2D);
	vector<double>().swap(Temp.ita_2D);
	Temp.inf_2D.resize(Temp.TOTAL_TIME_STEPS, 0.0);
	Temp.ita_2D.resize(Temp.TOTAL_TIME_STEPS, 0.0);

	Cal_Inf_2D(Temp);
	Cal_Ita_2D(Temp);											//ita_2D
}
/********************************************************************************************************************************
** Function name:       Cal_Inf_2D
** Descriptions:        Cal_Inf_2D
** input parameters:    Temp
** output parameters:   None
** Returned value:      None
********************************************************************************************************************************/
void Cal_Inf_2D(TEMP& Temp)
{
	double tmp1;
	double tmp2;
	...
	{
		tmp1 = pow((E_VALUE), -pow(((double)i)*timeStep / g_Spd.BETA_INF_2D, g_Spd.ALPHA_INF_2D));
		tmp2 = pow((E_VALUE), -pow(((double)(i + 1))*timeStep / g_Spd.BETA_INF_2D, g_Spd.ALPHA_INF_2D));
		Temp.inf_2D[i] = tmp1 - tmp2;
	}
}
/********************************************************************************************************************************
...
** Descriptions:        根据ita_2D（通过inf_2D可得出），recSurvival和deltaEff即可算出lamEff_2D
** input parameters:    Temp
** output parameters:   None
** Returned value:      None
********************************************************************************************************************************/
double Cal_LamEff_2D(TEMP& Temp)
{
...
** Descriptions:        根据ita_1D（通过inf_1D可得出）和recSurvival即可算出lamEff_1D
** input parameters:    Temp
** output parameters:   lamEff_1D
** Returned value:      lamEff_1D
********************************************************************************************************************************/
double Cal_LamEff_1D(TEMP& Temp)
{
	...
	for (int i = 0; i < Temp.TOTAL_TIME_STEPS; i++)
	{
		lamEff_1D += Temp.ita_1D[i] * Temp.recSurvival[i];		//ita_1D[i]值已经自带了dt
	}
...
** Descriptions:        计算inf_1D，recSurvival，deltaEff，ita_1D

						（备注：这里没有计算inf_2D和ita_2D，因为之后要不断更新parm值来找对应的lamEff，
						所以是在那时候通过调用 Update_Var_Of_Temp() 和 Cal_LamEff_2D() 才计算这个值，
						然后求出对应的lamEff）
** input parameters:    Temp
** output parameters:   None
** Returned value:      None
...
void Output_LamEff1D_To_File(double lamEff_1D)
{
	...
	sprintf(name, "task%d_lamEff_1D.csv", g_taskNum);
	fp = fopen(name, "w");
	fprintf(fp, "lamEff_1D,alphaI_1D,betaI_1D,alphaR,betaR\n");
	fprintf(fp, "%f,%f,%f,%f,%f\n", lamEff_1D, g_Spd.ALPHA_INF_1D, g_Spd.BETA_INF_1D, g_Spd.ALPHA_REC, g_Spd.BETA_REC);
	fclose(fp);
	free(name);
}
/********************************************************************************************************************************
...
void Output_LamEff2D_To_File(void)
{
	...
	name = (char *)malloc(NAME_SIZE * sizeof(char));
	sprintf(name, "task%d_lamEff_2D_parm.csv", g_taskNum);
	fp = fopen(name, "w");
	for (auto it = g_Spd.lamEff_Parm[0].begin(); it != g_Spd.lamEff_Parm[0].end(); it++)
	{
		fprintf(fp, "%f,%f\n", *it, g_Spd.lamEff_Parm[1][index]);
