...
{
	double x1 = 1.0 + 1.0 / g_Fun.ALPHA_REC;
	double x2;
	double aveRec_Rate = 1.0 / (g_Fun.BETA_REC * Gamma_Fun1(x1));		//set_parms.cpp中已经固定了平均恢复速率的值
	double temp;

	g_Fun.lamEff_Parm.resize(2);
#ifdef PARM_INF_2D
	x2 = 1.0 + 1.0 / g_Fun.ALPHA_INF_2D;
#elif defined PARM_INF_1D
	x2 = 1.0 + 1.0 / g_Fun.ALPHA_INF_1D;
#endif
	for (double effDif_Rate = (LAMEFF_MIN); effDif_Rate < ((LAMEFF_MAX)+((0.001)*(LAMEFF_DELTA))); effDif_Rate += (LAMEFF_DELTA))
	{
		...
		g_Fun.lamEff_Parm[0].push_back(effDif_Rate);
		g_Fun.lamEff_Parm[1].push_back(temp);		//beta_inf
	}
	...
	fprintf(fp, "%f,%f,%f,%f,%f,%f\n", 1.0 / (g_Fun.BETA_INF_1D * Gamma_Fun1(1.0 + 1.0 / g_Fun.ALPHA_INF_1D)),
		g_Fun.ALPHA_INF_1D, g_Fun.BETA_INF_1D, aveRec_Rate, g_Fun.ALPHA_REC, g_Fun.BETA_REC);
#elif defined PARM_INF_1D
	fprintf(fp, "aveInf_Rate_2D,alphaI_2D,betaI_2D,aveRec_Rate,alphaR,betaR\n");
	fprintf(fp, "%f,%f,%f,%f,%f,%f\n", 1.0 / (g_Fun.BETA_INF_2D * Gamma_Fun1(1.0 + 1.0 / g_Fun.ALPHA_INF_2D)),
		g_Fun.ALPHA_INF_2D, g_Fun.BETA_INF_2D, aveRec_Rate, g_Fun.ALPHA_REC, g_Fun.BETA_REC);
#endif
	fclose(fp);
...
#endif
	for (auto it = g_Fun.lamEff_Parm[0].begin(); it != g_Fun.lamEff_Parm[0].end(); it++)
	{
		fprintf(fp, "%f,%f\n", *it, g_Fun.lamEff_Parm[1][index]);
		index++;
	}
	fclose(fp);
	free(name);
...
void Cal_LamEff_And_Parm(TEMP& Temp)
{
	...
	isNear_lamEff_Min = (fabs(curLamEff - lamEff_Min) < fabs(curLamEff - lamEff_Max)) ? 1 : 0;
	...
	do
	{
		...
		g_Fun.lamEff_Parm[0].push_back(lamEff_Sol);
		g_Fun.lamEff_Parm[1].push_back(parmSol);
...
void Update_Var_Of_Temp(TEMP& Temp, double parm)
{
	...
	g_Fun.BETA_INF_1D = parm;
#elif defined PARM_BETA_INF_2D
	g_Fun.BETA_INF_2D = parm;
#endif
...
void Cal_Inf_1D(TEMP& Temp)
{
	...
	{
		tmp1 = pow((E_VALUE), -pow(((double)i)*timeStep / g_Fun.BETA_INF_1D, g_Fun.ALPHA_INF_1D));
		tmp2 = pow((E_VALUE), -pow(((double)(i + 1))*timeStep / g_Fun.BETA_INF_1D, g_Fun.ALPHA_INF_1D));
...
void Cal_Var_Of_Temp(TEMP& Temp)
{
	...
	for (int i = 0; i < Temp.TOTAL_TIME_STEPS; i++)
	{
		tmp1 = pow((E_VALUE), -pow(((double)(i))*timeStep / g_Fun.BETA_INF_2D, g_Fun.ALPHA_INF_2D));		//inf_2D
		tmp2 = pow((E_VALUE), -pow(((double)(i + 1))*timeStep / g_Fun.BETA_INF_2D, g_Fun.ALPHA_INF_2D));
		Temp.inf_2D[i] = tmp1 - tmp2;																	//这个已经自带dt了，因为是i的量减去i+1的
		//这里的tau~tau+dt时间内感染的概率inf_2D(即psi_inf*dt)是用相邻时间的存活概率相减得到的

		tmp1 = pow((E_VALUE), -pow(((double)i)*timeStep / g_Fun.BETA_REC, g_Fun.ALPHA_REC));			//recSurvival probability
...
void Output_LamEff2D_To_File(double lamEff_2D)
{
	...
	fprintf(fp, "%f,%f,%f,%f,%f\n", lamEff_2D, g_Fun.ALPHA_INF_2D, g_Fun.BETA_INF_2D, g_Fun.ALPHA_REC, g_Fun.BETA_REC);
	fclose(fp);
	free(name);
}
/********************************************************************************************************************************
