...
/********************************************************************************************************************************
** Function name:       Output_BetaC_Of_Diff_Rho0_To_File
** Descriptions:        Output_BetaC_Of_Diff_Rho0_To_File
** input parameters:    netIndex,betaC_Rho0
** output parameters:   None
** Returned value:      None
********************************************************************************************************************************/
void Output_BetaC_Of_Diff_Rho0_To_File(int netIndex, vector<vector<double> >& betaC_Rho0)
{
	...
	sprintf(name, "task%d_net%d_BetaC_Of_DiffRho0.csv", g_taskNum, netIndex);  
	fp = fopen(name, "w");
	fprintf(fp, "x,y1,y2\n");
	for (vector<vector<double> >::iterator it1 = betaC_Rho0.begin(); it1 != betaC_Rho0.end(); it1++)
	{
...
void Output_AveSteadyRho_Parm_To_File(double rho0, vector<vector<double> >& aveStRho_Parm)
{
	...
	name = (char *)malloc(NAME_SIZE * sizeof(char));
	sprintf(name, "task%d_AveSteadyRho_Parm_Rho0=%f.csv", g_taskNum, rho0);
	fp = fopen(name, "w");
